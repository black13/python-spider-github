...
headers = {"Authorization": "token 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"}
listspath = r'TopLists' 
...
def run_query(max_stars):
    ...
    while end_cursor != "":
        ...
        query = f"""
        {{
          search(query: "qt language:{LANGUAGE} fork:false pushed:>{LAST_ACTIVE} sort:stars stars:<{max_stars}", type: REPOSITORY, first: 100 {', after: "' + end_cursor + '"' if end_cursor else ''}) {{
            edges {{
              node {{
                ... on Repository {{
                  url
...
def main():
    newpath=os.path.join(os.path.abspath(''),listspath)

    if not os.path.isdir(newpath):
        os.makedirs(newpath) 
    
    ret=run_query(next_max_stars)
    print(ret)    
